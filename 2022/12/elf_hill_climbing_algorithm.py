...
class Grid:
    ...
    def _char_grid(self) -> list[list[str]]:
        ...
        out = [[height_to_char[height] for height in row] for row in self.grid]
    ...
    def print(self, *points: Point) -> None:
        ...
        for point in points:
            grid[point[0]][point[1]] = (
                Colors.cyan + grid[point[0]][point[1]] + Colors.default
            )
...
class Path:
    ...
    def chart(self) -> dict[Point, str]:
        ...
        if (
            not inner_path
        ):  # Empty inner path, in case path contains only start and/or end
            return {}
...
class Seeker:
    ...
    def get_step(self, point: Point) -> Optional[Point]:
        steps = [
            step
            for step in self.grid.cardinals(point)
            if step not in self.walked
            and self.grid.elevation(point, step) <= CLIMB_HEIGHT
        ]
    ...
    def metric(self, point: Point) -> int:
        ...
        ncol = self.grid.ncol
        tot = nrow + ncol
        return (nrow / tot) * abs(point[0] - self.grid.end[0]) + (ncol / tot) * abs(
            point[1] - self.grid.end[1]
        )
    ...
    def get_loopback(self, index: int) -> Optional[Point]:
        loopbacks = [
            point
            for point in self.grid.cardinals(self.path[index])
            if point in self.path[: index - 1]
            and self.grid.elevation(point, self.path[index]) <= CLIMB_HEIGHT
        ]
...
class Dijkstra:
    ...
    def _unvisited_points(self) -> Iterator[Point]:
        return (
            point for point in self.grid.points if self.nodes[point].visited == False
        )

    def _visited_points(self) -> Iterator[Point]:
        return (
            point for point in self.grid.points if self.nodes[point].visited == True
        )

    def _closest_point(self, points: Iterator[Point]) -> int:
        try:
            return min(points, key=lambda point: self.nodes[point].distance)
        except ValueError:
    ...
    def _unvisited_neighbours(self, point: Point) -> Iterator[Point]:
        return (
            neighbour
            for neighbour in self.grid.cardinals(point)
    ...
    def search(self) -> Path:
        self.nodes = {point: DijkstraNode() for point in self.grid.points}
        ...
        while (
            ...
            and self.nodes[self._closest_point(self._unvisited_points())].distance
            != math.inf
        ):
    ...
    def reverse_search(self) -> Path:
        ...
        while (
            self.nodes[self._closest_point(self._unvisited_points())].distance
            != math.inf
        ):
            current = self._closest_point(self._unvisited_points())
            for neighbour in self._unvisited_neighbours(current):
        ...
        return Path(
            Grid(self.grid.grid, start, self.grid.end), self._backtrack_path(start)
        )
